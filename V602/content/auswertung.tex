...
\begin{align*}
   \Theta_{K_\alpha} &= \input{build/Theta_k_kante_a.tex}  \\
   \Theta_{K_\beta} &= \input{build/Theta_k_kante_b.tex} \\
\end{align*}
...
\begin{align*}
  E_{K_\alpha} &= \input{build/E_k_kante_a_cu.tex} & E_{K_\alpha, \text{lit}} &= \input{build/E_k_kante_a_lit_cu.tex} \\
  E_{K_\beta} &= \input{build/E_k_kante_b_cu.tex} & E_{K_\beta, \text{lit}} &= \input{build/E_k_kante_b_lit_cu.tex} .
\end{align*}
...
\begin{align*}
  \lambda_{\text{min,t}} = \input{build/lambda_min_t.tex}
\end{align*}
beziehungsweise eine maximalen Energie von
\begin{align*}
  E_{\text{max,t}} = \input{build/E_max_t.tex}
\end{align*}
erwartet.
...
\begin{align*}
  \sigma_{1} &= \input{build/sigma_1_cu.tex} & \sigma_{1, \text{lit}} &= \input{build/sigma_1_lit_cu.tex} \\
  \sigma_{2} &= \input{build/sigma_2_cu.tex} & \sigma_{2, \text{lit}} &= \input{build/sigma_2_lit_cu.tex} \\
  \sigma_{3} &= \input{build/sigma_3_cu.tex} & \sigma_{3, \text{lit}} &= \input{build/sigma_3_lit_cu.tex}
\end{align*}
...
\begin{align*}
  \increment \Theta_1 &= \input{build/delta_theta_1.tex} \\
  \increment E_1 &= \input{build/delta_Energie_1.tex} \\
\end{align*}
und für den zweiten Peak Auflösungswerte von
\begin{align*}
  \increment \Theta_2 &= \input{build/delta_theta_2.tex} \\
  \increment E_2 &= \input{build/delta_Energie_2.tex}. \\
\end{align*}
...
\begin{align*}
  E_\text{Ge} &= \input{build/E_ge.tex}, \\
  E_\text{Zr} &= \input{build/E_zr.tex}, \\
  E_\text{Sr} &= \input{build/E_sr.tex}. \\
\end{align*}
Hieraus ergeben sich nach Formel \eqref{eqn:std} die Abschirmzahlen
\begin{align*}
  \sigma_\text{Ge} &= \input{build/sigma_ge.tex}, \\
  \sigma_\text{Zr} &= \input{build/sigma_zr.tex}, \\
  \sigma_\text{Sr} &= \input{build/sigma_sr.tex}. \\
\end{align*}
Die Literaturwerte, welche anhand der Energien der K-Kante aus der Literatur \cite{energie} berechnet werden, sind
\begin{align*}
  \sigma_\text{Ge, lit} &= \input{build/sigma_ge_lit.tex}, \\
  \sigma_\text{Zr, lit} &= \input{build/sigma_zr_lit.tex}, \\
  \sigma_\text{Sr, lit} &= \input{build/sigma_sr_lit.tex}. \\
\end{align*}
...
\begin{align*}
  R_y &= \input{build/ebberg_konstante.tex}, \\
\end{align*}
und darauf unverzüglich auf die Rydbergkonstante von
\begin{align*}
  R_{\infty} &= \input{build/ebberg_konstante_ry.tex}, \\
\end{align*}
geschlossen werden.
...
\begin{align*}
  \Theta_{\text{Wi},1} &= \input{build/theta_wi_1.tex}, \\
  \Theta_{\text{Wi},2} &= \input{build/theta_wi_2.tex} \\
\end{align*}
entnehmen.
Hieraus folgt eine Energiedifferenz von
\begin{align*}
  \increment E_\text{L} &= \input{build/wi_delta_E.tex}. \\
\end{align*}
Aus der angegebenen Formel folgt hiermit für die Abschirmkonstante von Wismut
\begin{align*}
  \sigma_{\text{Wi}} &= \input{build/sigma_wi.tex}. \\
\end{align*}
