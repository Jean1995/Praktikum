...
Bei dieser mit Wechselstrom betriebenen Schaltung wird das Verhältnis von $R_3$ zu $R_4$ nun solange variiert bis die Brückenspannung $U_{\text{Br}}$ ihr Minimum erreicht hat.
...
Dementsprechend berechnet sich der Fehler von $R_x$ zu
\begin{equation}
...
\begin{figure}[H]
  ...
  \label{fig:2}
\end{figure}
Sie unterscheidet sich von der Wheatstonebrücke nur darin, dass hier anstelle von $R_x$ eine unbekannte RC-Kombination oder eben ein unbekannter verlustbehafteter Kondensator eingebaut und betrachtet wird.
...
Auf diese Weise werden die Kapazitäten und Widerstände einer RC-Kombination, sowie von zwei unbekannten verlustbehafteten Kondensatoren bestimmt.
...
\end{align*}
Die Fehler bestimmen sich analog zu der in Gleichung \ref{eqn:fehler1} angegebenen Fehlerformel.
...
\subsection{Durchführung}
Die Induktivitätsmessbrücke, dargestellt in Abbildung \ref{fig:3}, funktioniert analog zur Kapazitätsmessbrücke in Abbildung \ref{fig:2}.
\begin{figure}[H]
  \centering
...
\end{equation}
für verschwindende Brückenspannung $U_{\text{Br}}$.
...
Dabei soll dieselbe LR-Kombination 19 untersucht werden, indem $R_3$ und $R_4$ nacheinander variiert werden, sodass die Brückenspannung $U_{\text{Br}}$ minimal wird.
...
Bei der Wien-Robinson-Brücke, beschrieben in Abbildung \ref{fig:5}, hingegen ist ein Abgleich nur bei einer bestimmten Frequenz möglich, die untersucht werden soll.
\begin{figure}[H]
  \centering
...
\end{equation}
Dieses Verhältnis wird nun in Abbildung \ref{fig:6} halblogarithmisch dargestellt.
%\begin{equation}
%  \omega = 2 \pi v
%\end{equation}
...
Die errechnete Sperrfrequenz, welche für das Plotten der Theoriekurve verwendet wird, beträgt nach Gleichung \ref{doof} jedoch $v_0 = \SI{1149.6}{\hertz}$.
...
Für diese gilt
\begin{equation}
  U_2 = \frac{U_{\text{Br}}}{f(\Omega = 2)} = \SI{0.0698}{\volt}.
\end{equation}
